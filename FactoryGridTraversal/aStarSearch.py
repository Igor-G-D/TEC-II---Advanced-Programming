...
def get_valid_neighbors(grid: List[List[int]], position: Tuple[int, int], allow_diagonals) -> List[Tuple[int, int]]:
    x, y = position
    moves = [
        (x + 1, y), (x - 1, y), # cardinal directions
        (x, y + 1), (x, y - 1),
    ]
    if allow_diagonals:
        moves.extend([(x + 1, y + 1), (x - 1, y - 1),(x + 1, y - 1), (x - 1, y + 1)]) # diagonals
    valid_neighbors = []
    for nx, ny in moves:
...
def a_star_search(grid: List[List[int]], start: Tuple[int, int], goal: Tuple[int, int], allow_diagonals) -> None:
    rows = len(grid)
    cols = len(grid[0])
    ...
    while open_list:
        ...
        # Explore neighbors
        for (nx, ny) in get_valid_neighbors(grid, (x, y), allow_diagonals):
