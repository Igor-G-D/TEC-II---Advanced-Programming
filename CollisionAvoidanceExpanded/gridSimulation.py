...
from classes import Simulation, RectangleGrid, StepCommand, StepBackCommand
...
# logs
path_size = []
robots_progress = [] # stores when each robot had to wait for others to move
overhead_ms = [] # calculates the overhead for the look ahead helper method

def clearSimulation(clearData=False):
    global simulation, path_size, main_image, cell_shape
    print("cleared!")
    # Reset simulation
    factory = DefaultSimulationFactory()
    simulation = Simulation(
    ...
    reset_window(main_image, simulation.robots, simulation.goals, simulation.get_paths())
    ...
    if event == cv2.EVENT_RBUTTONDOWN:
        ...
        if not simulation.grid.is_obstacle(cell_clicked):
            if len(simulation.robots) > len(simulation.goals):
                ...
                simulation.add_robot(cell_clicked, algorithms[algorithm_index % len(algorithms)], avoidance_methods[avoidance_method_index % len(avoidance_methods)])
...
def reset_window(img, robots, destinations, paths):
    ...
    for i, robot in enumerate(robots):
        paint_cell(img, robot.get_curr_pos(), bgr_palette[i], 0, cell_shape)
    ...
    if running:
        ...
        if not pressed:
            if key == 27 and not pressed:  # ESC to exit
                ...
                command = StepCommand(simulation)
                simulation.run_command(command)
                reset_window(main_image, simulation.robots, simulation.goals, simulation.get_paths())
            elif key == 9: # backspace
                simulation.undo()
                reset_window(main_image, simulation.robots, simulation.goals, simulation.get_paths())
        else:
            if key == 255:
...
# 1. Get detailed logs from the simulation instance
robot_logs = simulation.get_robot_log_data()

# 2. Build the consolidated performance data
performance_data = []
for i in range(len(simulation.robots)):
    robot = simulation.robots[i]
    goal = simulation.goals[i] if i < len(simulation.goals) else None
    
    overhead_list = robot_logs.get(i, {}).get("overhead", [])
    
    performance_data.append({
        'robot_id': i,
        'start_position': robot.position,
        'goal_position': goal.position if goal else None,
        'path_size': path_size[i] if i < len(path_size) else 0,
        'movement_history': robot.movement_history,
        'total_waiting_steps': robot.movement_history.count(False),
        'individual_overhead': overhead_list,
        'total_overhead_ms': sum(overhead_list) * 1000 
    })
