    ...
    def on_event(self, event: Event):
        if event.event_type == EventType.MOVEMENT:
            ...
            if other_robot != self and (this_robot_moved or other_robot_moved):
                ...
                if distance == 0:
                    print(f"**COLLISION (Distance 0):** Robot {self.id} at {my_position} collided with robot {other_robot.id}!")
                elif distance <= 3.0:
                    print(f"**PROXIMITY WARNING (Distance <= 3):** Robot {self.id} at {my_position} is close to robot {other_robot.id} (Distance: {distance:.1f})!")
